...
| Migrations | Alembic |
| Auth provider | Local HS256 JWT + bcrypt (self-contained) |
| Database | Supabase PostgreSQL (hosted, via transaction pooler) |
...
### Auth flow
Supabase Auth has been fully removed. Auth is now self-contained in the backend.

- **Login**: `POST /api/auth/login` → bcrypt password verify → sets two httpOnly cookies:
  - `access_token`: HS256 JWT, 1-hour TTL, path `/`
  - `refresh_token`: HS256 JWT, 30-day TTL, path `/api/auth/refresh`
- **Every API call**: browser sends cookies automatically (`credentials: 'include'`); no Authorization header.
- **Token verification** (`get_current_user`): reads `access_token` cookie, decodes HS256 JWT with `JWT_SECRET_KEY`, looks up user in DB.
- **Token refresh** (`POST /api/auth/refresh`): reads `refresh_token` cookie, issues new `access_token` cookie.
- **Auto-refresh** in `api.ts`: on 401, attempts refresh once, then retries the original request; if refresh also fails, redirects to `/auth/login`.
- **Logout**: `POST /api/auth/logout` clears both cookies.
- **Route protection**: Next.js `proxy.ts` (Next.js 16 rename of `middleware.ts`) checks for `access_token` cookie presence; RSC layout double-checks before rendering.
- **Password hashing**: `bcrypt` (5.x) directly — no passlib wrapper.
- **Signup migration**: if a user registers with an existing email that has no `password_hash` (Supabase-seeded accounts), the endpoint sets the hash in-place, preserving the UUID and all linked household data.
- **Re-auth for destructive actions** (Settings → Danger Zone): `POST /api/auth/verify-password` with `{password}` — returns 200 or 401.
- `JWT_SECRET_KEY` in `backend/.env` — a 128-char hex string. Never committed.
...
Historical: was needed for Supabase Python client and PyJWKClient. Both have been removed.
The new auth stack (bcrypt + PyJWT) is synchronous but fast enough to run directly in async
handlers without event loop contention. No thread executors needed for auth anymore.
...
**Core**
- `users` — app user records; `id` is a UUID generated by the backend (previously Supabase Auth UUID — preserved for existing accounts); `password_hash` added in migration `i9j0k1l2m3n4` (nullable for migration compatibility)
...
- Frontend: `POST /api/auth/verify-password` re-auth + type `DELETE` confirmation required before the request is sent
...
| 2026-08-25 | Auth | Decoupled Supabase Auth: local HS256 JWT (PyJWT) + bcrypt; httpOnly cookies; new `/api/auth/*` router; migration i9j0k1l2m3n4 adds `password_hash`; proxy.ts replaces middleware.ts (Next.js 16) |
